...
private:
    ...
    bool isDeviceSuitable(VkPhysicalDevice device)
    {
        ...
        {
            // Swap chain support is sufficient for now if there is at least one supported image format and presentation mode given the window surface we have
    ...
    void createSwapChain()
    {
        ...
        // We have to decide how many images we would like to have in the swap chain. The implementation specifies the minimum number that it requires to function
        ...
        // Hence, it is recommended to request at least one more image than the minimum
        ...
        // We don't want any transformation to be applied to the images in the swap chain, so we'll simply use the current transform
        createInfo.preTransform = swapChainSupport.capabilities.currentTransform;

        // We don't want any blending with other windows, so we'll ignore the alpha channel
    ...
    void createImageViews()
    {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++)
        {
            ...
            // We need to specify how the image data should be interpreted
    ...
    {
        // We will have just one color buffer attachment represented by one of the images from the swap chain
        // The format should be the same as the one used in the swap chain
        VkAttachmentDescription colorAttachment{};
        colorAttachment.format = swapChainImageFormat;
        colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT;

        // The loadOp and storeOp determine what to do with the data in the attachment before rendering and after rendering
        // We are clearing the framebuffer to black before drawing a new frame
        colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
        colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        // Out application will not be doing anything with the stencil buffer, so we don't care about these
        colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        // Textures and framebuffers in Vulkan are represented by VkImage objects with a certain pixel format, but the layout of the pixels in memory can change
        // based on what you're going to do with the image
        // Images need to be transitioned to specific layouts that are suitable for the operation that we are specifying next
        // The initial layout is set as VK_IMAGE_LAYOUT_UNDEFINED to indicate that we don't care what previous layout the image was in, because we're going to clear it anyway
        // The final layout specifies the layout that the image will have after the render pass, in this case we want it to be ready for presentation using the swap chain
    ...
    {
        auto vertShaderCode = readFile("../../Examples/Shaders/Vert.spv");
        auto fragShaderCode = readFile("../../Examples/Shaders/Frag.spv");
        ...
        // The vertex input is the stage where we describe the format of the vertex data that will be passed to the vertex shader
        ...
        // The input assembly stage is where we describe what kind of geometry will be drawn from the vertices and if primitive restart should be enabled
        VkPipelineInputAssemblyStateCreateInfo inputAssembly{};
        inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; // We will be rendering separate triangles from the vertices
        ...
        // Examples are the size of the viewport, line width and blend constants. If you want to use dynamic state and keep these properties out, then you'll have to fill in a VkPipelineDynamicStateCreateInfo structure like this:
        ...
        // The pipeline layout is a Vulkan object that defines the set of resources that can be accessed by a pipeline, as uniform buffers, image samplers, etc
    ...
    void createSyncObjects()
    {
        ...
        // We create a signaled fence so that the application doesn't wait for the first image to be available before starting to render
